export const VALID_RANGES = ["1h", "1d", "7d", "30d"];
...
function isTimeRange(value) {
    return VALID_RANGES.includes(value);
}
...
export function buildActivityCountsQuery(range) {
    return `sum by(activity_type)(increase(temporal_activity_task_completed_total[${range}]))`;
}
export async function queryActivityCounts(prometheusUrl, range) {
...
export function buildWorkflowCountsQuery(range) {
    return `sum by(workflow_type)(increase(temporal_workflow_task_completed_total[${range}]))`;
}
export async function queryWorkflowCounts(prometheusUrl, range) {
